...
class ReportActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ReportView(intent, contentResolver)
        }
    }
}
...
@Composable
fun ReportView(intent: Intent, contentResolver: ContentResolver) {
    ...
    LaunchedEffect(Unit) {
        ...
        withContext(Dispatchers.IO) {
            ...
            if (fd != null && completeName != null) {
                report = Core.generateReport(fd.detachFd(), completeName)
            }
        }
        isLoading = false
    }
...
@Composable
fun Report(isLoading: Boolean, report: String?, fileName: String?) {
    Surface(
...
@Composable
fun WebViewComposable(report: String) {
    val context = LocalContext.current
